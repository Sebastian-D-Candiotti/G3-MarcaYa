...
7. [Evolución de Patrones — Guía de Refactorización](#7-evolución-de-patrones--guía-de-refactorización)
   - 7.1 [Patrón Strategy — Validación de Marcación](#71-patrón-strategy--validación-de-marcación)
   - 7.2 [Patrón Factory — Creación de Registros](#72-patrón-factory--creación-de-registros)
   - 7.3 [Patrón Command — Desacoplamiento de Acciones](#73-patrón-command--desacoplamiento-de-acciones)
   - 7.4 [Patrón Facade — Interfaz Única de Sistema](#74-patrón-facade--interfaz-única-de-sistema)
   - 7.5 [Resumen para el Agente de IA](#75-resumen-para-el-agente-de-ia)
   - 7.6 [Patrón Strategy — Procesamiento de Pagos](#76-patrón-strategy--procesamiento-de-pagos-suscripciones)
...
│   │           ├── suscripciones_controller.rb     # Contratar / cancelar / consultar suscripción
...
│   │       └── i_gestionar_suscripcion.rb          # interface: contratar / cancelar / verificar suscripción
│   │
│   ├── # ─────────────────────────────────────────────────
...
│   │   │   └── sistema_asistencia_facade.rb        # Facade unificada: suscripción + validación + registro
│   │   │
│   │   ├── commands/                               # Command pattern - encapsulan intenciones del usuario
│   │   │   └── marcar_entrada_command.rb           # Ej: MarcarEntradaCommand.new(params).execute
│   │   │
...
│   │           ├── contratar_plan.rb                  # Crea Suscripcion, valida límites
│   │           ├── cancelar_suscripcion.rb            # Cambia estado a vencida
│   │           ├── ver_suscripcion_activa.rb          # Consulta suscripción vigente
│   │           └── listar_planes.rb                   # Consulta catálogo PlanSuscripcion
│   │
│   ├── # ─────────────────────────────────────────────────
...
│   │   │   ├── plan_suscripcion.rb                 # Catálogo: Bronce/Plata/Oro, precio, limite_empleados
│   │   │   └── suscripcion.rb                      # Vincula Empresa + Plan, controla vigencia
│   │   │
...
│   │   │
│   │   ├── strategies/                             # Strategy pattern - validaciones intercambiables
│   │   │   ├── i_validacion_strategy.rb            # Interfaz: validar(datos)
│   │   │   ├── gps_strategy.rb                     # Validación por geocerca (Haversine)
│   │   │   └── contexto_validacion.rb              # Contexto que selecciona la estrategia activa
│   │   │
...
│   │       ├── i_plan_suscripcion_repository.rb    # Puerto para catálogo de planes
│   │       ├── i_suscripcion_repository.rb         # Puerto para suscripciones activas
...
│   │   │   ├── ar_plan_suscripcion_repository.rb   # Implementa IPlanSuscripcionRepository
│   │   │   └── ar_suscripcion_repository.rb        # Implementa ISuscripcionRepository
│   │   │
...
│   │   │   ├── plan_suscripcion_record.rb          # Mapeo tabla plan_suscripciones
│   │   │   └── suscripcion_record.rb               # Mapeo tabla suscripciones
│   │   │
...
│   │   │   ├── plan_suscripcion_mapper.rb          # PlanSuscripcionRecord ↔ PlanSuscripcion
│   │   │   └── suscripcion_mapper.rb               # SuscripcionRecord ↔ Suscripcion
│   │   │
│   │   ├── factories/                              # Factory pattern - creación especializada
│   │   │   └── registro_factory.rb                 # build(:entrada/salida, params)
│   │   │
...
│       ├── plan_suscripcion_serializer.rb
│       └── suscripcion_serializer.rb
│
├── config/
...
│   │   ├── 012_create_plan_suscripciones.rb         # Catálogo de planes (Bronce/Plata/Oro)
│   │   └── 013_create_suscripciones.rb              # Suscripciones activas por empresa
│   └── schema.rb
│
...
**PlanSuscripcion** (catálogo): `id`, `nombre (Bronce|Plata|Oro)`, `precio`, `limiteEmpleados`, `descripcion`

**Suscripcion** (vincula Empresa + Plan): `id`, `empresaId`, `planId`, `fechaInicio`, `fechaFin`, `estado (activa|vencida)`
...
**Suscripciones:** `GET /suscripciones/planes`, `GET /suscripciones/planes/:id`, `POST /suscripciones/contratar`, `GET /suscripciones/mi-suscripcion`, `PUT /suscripciones/:id/cancelar`
...
* Biometría como método de validación de asistencia (solo aplica a autenticación de inicio de sesión)  
...
### **1.12 PlanSuscripcion *(catálogo de planes)***
...
| `limiteEmpleados` | Integer | Máximo de empleados activos permitidos |
| `descripcion` | String | Descripción del plan |
| `fechaCreacion` | DateTime | Fecha de creación del registro |

---

### **1.13 Suscripcion *(vincula Empresa con un Plan)***
...
| Empresa | 1 : N | Suscripcion | Una empresa puede tener múltiples suscripciones en el tiempo |
| PlanSuscripcion | 1 : N | Suscripcion | Un plan puede estar contratado por muchas empresas |
| Empresa | 1 : 1 | Suscripcion *(activa)* | Una empresa tiene una sola suscripción activa a la vez |
...
| Método | Endpoint | Rol | Descripción |
| :---- | :---- | :---- | :---- |
| `GET` | `/suscripciones/planes` | Público | Listar catálogo de planes disponibles |
| `GET` | `/suscripciones/planes/:id` | Público | Ver detalle de un plan |
| `POST` | `/suscripciones/contratar` | EMPRESA | Contratar un plan (`planId`, `periodo`) |
| `GET` | `/suscripciones/mi-suscripcion` | EMPRESA | Ver suscripción activa de la empresa |
| `PUT` | `/suscripciones/:id/cancelar` | EMPRESA / ADMIN | Cancelar / dar de baja una suscripción |
...
| Suscripciones | PlanSuscripcion, Suscripcion | 5 |
| Empleados (admin) | Empleado | 6 |
| **TOTAL** | **13 entidades** | **\~63 endpoints** |
...
```ruby
# app/domain/entities/suscripcion.rb
module Domain
  module Entities
    class Suscripcion
      attr_reader :id, :empresa_id, :plan_id, :estado, :fecha_inicio, :fecha_fin, :plan

      def initialize(id:, empresa_id:, plan_id:, estado:, fecha_inicio:, fecha_fin:, plan: nil)
        @id          = id
      ...
      def activa?
        @estado == 'ACTIVA' && @fecha_fin > Time.current
      end

      def puede_agregar_empleado?(conteo_actual)
        return true unless @plan
        conteo_actual < @plan.limite_empleados
      end
    end
  end
end
...
```ruby
# app/domain/entities/plan_suscripcion.rb
module Domain
  module Entities
    class PlanSuscripcion
      attr_reader :id, :nombre, :precio, :limite_empleados, :descripcion, :fecha_creacion

      def initialize(id:, nombre:, precio:, limite_empleados:, descripcion:, fecha_creacion: nil)
        @id               = id
        @nombre           = nombre
        @precio           = precio
        @limite_empleados = limite_empleados
        @descripcion      = descripcion
        @fecha_creacion   = fecha_creacion
      end

      def to_s
        "#{@nombre} — #{@limite_empleados} empleados ($#{@precio})"
      end
    end
  end
end
...
| Asistencia | `AsistenciaFacade` → `SistemaAsistenciaFacade` *(evolución)* | `MarcarEntrada`, `MarcarSalida`, `ObtenerHistorial`, `ObtenerTiempoReal` — también `MarcarEntradaCommand` (Command), `RegistroFactory` (Factory), `ContextoValidacion` + `GpsStrategy` (Strategy) | RegistroAsistencia, Parada, RegistroEntrada, RegistroSalida | 6 |
...
| Suscripciones | — | `ContratarPlan`, `CancelarSuscripcion`, `VerSuscripcionActiva`, `ListarPlanes` | PlanSuscripcion, Suscripcion | 5 |
...
module Domain
  module Errors
    ...
    class SuscripcionVencidaError  < StandardError; end
    class LimiteEmpleadosExcedidoError < StandardError; end
    class PlanNoEncontradoError   < StandardError; end
  end
end
```
...
## 7. Evolución de Patrones — Guía de Refactorización

> **Propósito:** Esta sección documenta cómo evolucionar la arquitectura Hexagonal actual hacia una estructura que soporte los patrones **Strategy, Factory, Command y Facade** sin romper la integridad del sistema. Está diseñada para que un **Agente de IA** o un **Desarrollador** comprenda el orden y la dirección de los cambios.

### 7.1 Patrón Strategy — Validación de Marcación

**Contexto:** Actualmente la validación GPS está acoplada directamente al caso de uso (`MarcarEntrada#ejecutar`). El objetivo es permitir que en el futuro se activen métodos alternativos (QR, Biometría) sin tocar el Interactor.

#### 🔴 Antes (Referencia Actual)

La lógica de GPS se invoca directamente desde el caso de uso:
...
def ejecutar(empleado_id:, parada_id:, latitud:, longitud:)
  ...
  # ⚠️ Acoplamiento directo: solo GPS, difícil de extender
  valida_gps = @gps_service.dentro_de_geocerca?(coordenada, parada.geocerca)
  raise Domain::Errors::FueraDeGeocercaError unless valida_gps
...
Se define una interfaz de estrategia pura en el dominio. El sistema elige la implementación según la configuración de la Parada.

```ruby
# app/domain/strategies/i_validacion_strategy.rb
module Domain
  module Strategies
    class IValidacionStrategy
      # @param datos [Hash] contiene coordenadas, parada, etc.
      # @return [Boolean]
      # @raise [Domain::Errors::ValidacionNoSoportadaError]
      def validar(datos)
        raise NotImplementedError
      end
    end
...
```ruby
# app/domain/strategies/gps_strategy.rb
module Domain
  module Strategies
    class GpsStrategy < IValidacionStrategy
      def validar(datos)
        coordenada = datos[:coordenada]
        geocerca   = datos[:geocerca]
        GpsValidationService.new.dentro_de_geocerca?(coordenada, geocerca)
      end
    end
  end
end
...
```ruby
# app/domain/strategies/contexto_validacion.rb
module Domain
  module Strategies
    class ContextoValidacion
      def initialize(tipo_verificacion, estrategia: nil)
        @estrategia = estrategia || STRATEGIAS.fetch(tipo_verificacion) {
          raise Domain::Errors::ValidacionNoSoportadaError, "Tipo: #{tipo_verificacion}"
        }
      end
...
El caso de uso ahora es agnóstico al método de validación:
...
def ejecutar(empleado_id:, parada_id:, latitud:, longitud:)
  ...
  contexto = Domain::Strategies::ContextoValidacion.new(parada.tipo_verificacion)
  valida_gps = contexto.validar!(
    coordenada: Domain::ValueObjects::CoordenadaGps.new(latitud, longitud),
    geocerca:   parada.geocerca
  )
  raise Domain::Errors::FueraDeGeocercaError unless valida_gps
...
### 7.2 Patrón Factory — Creación de Registros

**Contexto:** Diferenciar la lógica de creación entre un `RegistroEntrada` y un `RegistroSalida`. Cada tipo tiene reglas específicas (ej. la salida debe calcular `duracionJornada`).

#### 🔴 Antes (Referencia Actual)

Se crea un objeto genérico `RegistroAsistencia` y se le asigna un string `ENTRADA` o `SALIDA`:

```ruby
registro = Domain::Entities::RegistroAsistencia.new(
  tipo_marcacion:      'ENTRADA',   # 💣 string mágico
  empleado_id:         empleado.id,
  parada_id:           parada.id,
  fecha_hora:          Time.current,
  latitud_registrada:  latitud,
...
Se delega la responsabilidad de instanciación a una fábrica que asegura que cada tipo cumpla sus reglas:

```ruby
# app/domain/entities/registro_entrada.rb
module Domain
  module Entities
    class RegistroEntrada < RegistroAsistencia
      def initialize(params)
        super(params.merge(tipo_marcacion: 'ENTRADA'))
      end
    end
  end
end
...
```ruby
# app/domain/entities/registro_salida.rb
module Domain
  module Entities
    class RegistroSalida < RegistroAsistencia
      attr_reader :duracion_jornada

      def initialize(params)
        super(params.merge(tipo_marcacion: 'SALIDA'))
        @duracion_jornada = params[:duracion_jornada]
      end
    end
  end
end
...
```ruby
# app/infrastructure/factories/registro_factory.rb
module Infrastructure
  module Factories
    class RegistroFactory
      # @param tipo [String] 'ENTRADA' o 'SALIDA'
      # @param params [Hash] atributos del registro
      # @return [Domain::Entities::RegistroAsistencia]
      def self.build(tipo, params)
        case tipo
        when 'ENTRADA' then Domain::Entities::RegistroEntrada.new(params)
        when 'SALIDA'  then Domain::Entities::RegistroSalida.new(params)
        else raise ArgumentError, "Tipo de registro no soportado: #{tipo}"
        end
      end
    end
  end
...
### 7.3 Patrón Command — Desacoplamiento de Acciones

**Contexto:** Convertir las acciones del controlador en objetos `Command` que pueden ser encolados, auditados o revertidos.

#### 🔴 Antes (Referencia Actual)

El controlador llama directamente al Facade:

```ruby
# app/controllers/api/v1/asistencia_controller.rb
def marcar_entrada
  resultado = @marcar_asistencia_port.marcar_entrada(...)
  render json: RegistroAsistenciaSerializer.new(resultado).as_json, status: :created
end
...
Cada intención del usuario es un objeto que encapsula la petición y su ejecución:

```ruby
# app/application/commands/marcar_entrada_command.rb
module Application
  module Commands
    class MarcarEntradaCommand
      def initialize(api_params)
        @empleado_id = api_params[:empleado_id]
        @parada_id   = api_params[:parada_id]
        @latitud     = api_params[:latitud].to_f
        @longitud    = api_params[:longitud].to_f
      end

      def execute
        # Inyección de dependencias via inicializador global
        facade = Rails.configuration.sistema_asistencia_facade
        facade.registrar_entrada(@empleado_id, @parada_id, @latitud, @longitud)
      end

      def to_h
        { comando: 'marcar_entrada', empleado_id: @empleado_id, parada_id: @parada_id }
      end
    end
  end
end
...
# app/controllers/api/v1/asistencia_controller.rb
def marcar_entrada
  comando = Application::Commands::MarcarEntradaCommand.new(
    params.merge(empleado_id: current_user.id)
  )
...
### 7.4 Patrón Facade — Interfaz Única de Sistema

**Contexto:** Unificar el acceso a múltiples subsistemas (Asistencia, Suscripción, Notificaciones) bajo una sola fachada que el Frontend (Flutter) consume.

#### 🔴 Antes (Referencia Actual)

El controlador conoce demasiados servicios y el facade actual sólo orquesta el flujo de asistencia:

```ruby
# app/application/facades/asistencia_facade.rb
# Solo cubre marcación — no conoce suscripciones, pagos ni notificaciones
```

#### 🟢 Después (Con SistemaAsistenciaFacade)

La fachada actúa como el único punto de entrada para el frontend, ocultando si el sistema está validando una suscripción o enviando un correo al mismo tiempo que marca la asistencia:

```ruby
# app/application/facades/sistema_asistencia_facade.rb
module Application
  module Facades
    class SistemaAsistenciaFacade
      ...
      def initialize(
        ...
        asistencia_repo:  Infrastructure::Repositories::ArRegistroAsistenciaRepository.new,
        gps_service:      Domain::Services::GpsValidationService.new,
        notificacion_svc: Infrastructure::Services::MailerNotificacionService.new
      )
        @empleado_repo    = empleado_repo
        @parada_repo      = parada_repo
        @asistencia_repo  = asistencia_repo
        @gps_service      = gps_service
        @notificacion_svc = notificacion_svc
      end

      def registrar_entrada(empleado_id, parada_id, latitud, longitud)
        # 1. Validar suscripción activa de la empresa
        empleado = @empleado_repo.find_by_id!(empleado_id)
        # TODO: validar suscripción (fuera de alcance v1)

        # 2. Ejecutar Factory para crear el registro tipado
        comando = Application::Commands::MarcarEntradaCommand.new(
          empleado_id: empleado_id, parada_id: parada_id,
          latitud: latitud, longitud: longitud
        )
        registro = comando.execute

        # 3. Estrategia de validación
        parada = @parada_repo.find_by_id!(parada_id)
        contexto = Domain::Strategies::ContextoValidacion.new(parada.tipo_verificacion)
        valida = contexto.validar!(
          coordenada: Domain::ValueObjects::CoordenadaGps.new(latitud, longitud),
          geocerca:   parada.geocerca
        )

        # 4. Notificar si es necesario
        @notificacion_svc.notificar_marcacion(empleado, registro) if valida
...
### 7.5 Resumen para el Agente de IA

Para implementar estos cambios, seguí este orden de prioridad:

1. **Definí las interfaces Strategy** — Creá las clases abstractas en `app/domain/strategies/` (`IValidacionStrategy`, `GpsStrategy`, `ContextoValidacion`).
2. **Extraé la lógica de validación** — Mové el cálculo de GPS del `GpsValidationService` actual hacia `GpsStrategy`.
3. **Refactorizá el caso de uso** — Hacé que reciba un objeto de validación genérico a través del `ContextoValidacion` en lugar de llamar a `gps_service` directamente.
4. **Implementá la Factory** — Creá `RegistroFactory` y las subclases `RegistroEntrada` / `RegistroSalida` para eliminar strings mágicos.
5. **Envolvé en Commands** — Creá comandos para cada acción del controlador (`MarcarEntradaCommand`, `MarcarSalidaCommand`).
6. **Unificá en la Facade** — Evolucioná `AsistenciaFacade` a `SistemaAsistenciaFacade` como único punto de entrada para Flutter.

**Nota:** La validación biométrica (huella dactilar, reconocimiento facial) corresponde exclusivamente al flujo de **autenticación** como alternativa al login tradicional (email + contraseña). No tiene relación con la validación de asistencia ni forma parte del patrón Strategy aquí documentado.

---

### 7.6 Patrón Strategy — Procesamiento de Pagos (Suscripciones)

**Contexto:** El caso de uso `ContratarPlan` necesita procesar un pago a través de una API bancaria externa. Cada banco (BCP, BBVA, Interbank, Scotiabank) expone una API diferente con distintos endpoints, métodos de autenticación y formatos de solicitud.

#### Solución (Strategy como Driven Port)

La interfaz `IPagoStrategy` se define como un **Driven Port** porque la implementación concreta depende de servicios externos (bancos), a diferencia de `IValidacionStrategy` que es puramente de dominio.

```ruby
# app/ports/driven/i_pago_strategy.rb
module Ports
  module Driven
    module IPagoStrategy
      # @param empresa_id [String]
      # @param plan [Domain::Entities::PlanSuscripcion]
      # @param monto [Decimal]
      # @param moneda [String] 'PEN' por defecto
      # @return [Domain::ValueObjects::ResultadoPago]
      def procesar_pago(empresa_id:, plan:, monto:, moneda: 'PEN')
        raise NotImplementedError
      end
    end
  end
...
```ruby
# app/domain/value_objects/resultado_pago.rb
module Domain
  module ValueObjects
    class ResultadoPago
      attr_reader :exitoso, :id_transaccion, :fecha_procesado, :mensaje

      def initialize(exitoso:, id_transaccion: nil, fecha_procesado: nil, mensaje: nil)
        @exitoso         = exitoso
...
El `ContextoPago` vive en la capa de aplicación y selecciona la estrategia según el banco que la empresa eligió al contratar:

```ruby
# app/application/services/contexto_pago.rb
module Application
  module Services
    class ContextoPago
      STRATEGIAS = {
        'BCP'        => Infrastructure::Services::BcpPagoStrategy.new,
        'BBVA'       => Infrastructure::Services::BbvaPagoStrategy.new,
        'INTERBANK'  => Infrastructure::Services::InterbankPagoStrategy.new,
        'SCOTIABANK' => Infrastructure::Services::ScotiabankPagoStrategy.new
      }.freeze

      def procesar(banco:, empresa_id:, plan:, monto:, moneda: 'PEN')
        estrategia = STRATEGIAS.fetch(banco.upcase) {
          raise Domain::Errors::BancoNoSoportadoError, "Banco: #{banco}"
        }
        estrategia.procesar_pago(
          empresa_id: empresa_id, plan: plan,
...
Cada banco implementa el puerto con su lógica específica:

```ruby
# app/infrastructure/services/bcp_pago_strategy.rb
module Infrastructure
  module Services
    class BcpPagoStrategy
      include Ports::Driven::IPagoStrategy

      def procesar_pago(empresa_id:, plan:, monto:, moneda: 'PEN')
        # Llamada HTTP a la API de BCP
        # ... parsear respuesta
        Domain::ValueObjects::ResultadoPago.new(
          exitoso: true,
          id_transaccion: response[:id],
          fecha_procesado: Time.current
        )
      rescue HTTP::Error => e
        Domain::ValueObjects::ResultadoPago.new(
          exitoso: false, mensaje: e.message
        )
      end
    end
  end
...
El caso de uso `ContratarPlan` ahora inyecta `ContextoPago` en lugar de llamar directamente a un banco:

```ruby
# app/application/use_cases/suscripciones/contratar_plan.rb
module Application
  module UseCases
    module Suscripciones
      class ContratarPlan
        def initialize(
          suscripcion_repo: Infrastructure::Repositories::ArSuscripcionRepository.new,
          plan_repo:        Infrastructure::Repositories::ArPlanSuscripcionRepository.new,
          contexto_pago:    Application::Services::ContextoPago.new
        )
          @suscripcion_repo = suscripcion_repo
          @plan_repo        = plan_repo
          @contexto_pago    = contexto_pago
        end

        def ejecutar(empresa_id:, plan_id:, banco:, monto:)
          plan = @plan_repo.find_by_id!(plan_id)

          resultado = @contexto_pago.procesar(
            banco: banco, empresa_id: empresa_id,
            plan: plan, monto: monto
          )

          raise Domain::Errors::PagoRechazadoError, resultado.mensaje unless resultado.exitoso?

          suscripcion = Domain::Entities::Suscripcion.new(
            empresa_id: empresa_id, plan_id: plan_id,
            fecha_inicio: Time.current, fecha_fin: 1.month.from_now,
            estado: 'ACTIVA'
          )
          @suscripcion_repo.guardar(suscripcion)
        end
      end
    end
  end
...
```ruby
class Domain::Errors::BancoNoSoportadoError < StandardError; end
class Domain::Errors::PagoRechazadoError   < StandardError; end
```

---

### Convenciones de Nomenclatura — Nuevos Patrones

| Elemento | Convención | Ejemplo |
|----------|-----------|---------|
| Strategy interface (puro dominio) | `I` + proposito + `Strategy` | `IValidacionStrategy` |
| Strategy concreta (dominio) | proposito + `Strategy` | `GpsStrategy` |
| Contexto Strategy (dominio) | `Contexto` + proposito | `ContextoValidacion` |
| Driven Port Strategy (pagos) | `I` + proposito + `Strategy` | `IPagoStrategy` |
| Strategy concreta (pagos) | banco + `PagoStrategy` | `BcpPagoStrategy` |
| Contexto Strategy (aplicación) | `Contexto` + proposito | `ContextoPago` |
| Value Object (resultado) | `Resultado` + proposito | `ResultadoPago` |
| Command | intención + `Command` | `MarcarEntradaCommand` |
| Factory | entidad + `Factory` | `RegistroFactory` |
