...
class _MemoryRepository extends MarcacionPendienteRepository {
  ...
  Future<void> guardar(MarcacionPendiente marcacion) async {
    if (records.every(
      (item) => item.clienteMarcacionId != marcacion.clienteMarcacionId,
    )) {
...
void main() {
  ...
  test('offline mark is stored with original time and pending count', () async {
    ...
    expect(
      repository.records.single.estado,
      MarcacionPendiente.estadoPendiente,
    );
  });

  test(
    'partial success deletes only synchronized and duplicate records',
    () async {
      final repository = _MemoryRepository()
        ..records.addAll([
          _pending('ok'),
      ...
      final provider = AsistenciaOfflineProvider(
        ...
        apiService: ApiService.createForTesting(
          client: _Client((request) async {
            ...
            expect(
              sent.first['fecha_hora_original'],
      ...
      expect(repository.records.map((item) => item.clienteMarcacionId), [
        'failed',
      ]);
