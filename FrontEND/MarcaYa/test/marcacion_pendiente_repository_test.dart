...
void main() {
  ...
  test('deletes only backend-confirmed client ids', () async {
    await repository.guardar(marcacion('offline-ok', DateTime.utc(2026, 7, 12, 8)));
    await repository.guardar(marcacion('offline-failed', DateTime.utc(2026, 7, 12, 9)));

    await repository.eliminarPorClienteIds(['offline-ok']);

    final records = await repository.obtenerPendientes();
    expect(records.map((record) => record.clienteMarcacionId), ['offline-failed']);
  });

  test('records synchronization errors without deleting local row', () async {
    await repository.guardar(marcacion('offline-1', DateTime.utc(2026, 7, 12, 8)));
