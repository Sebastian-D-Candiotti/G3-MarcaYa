  ...
  return _MockHttpClient(
    handler: (request) async {
      ...
      if (times != null && callCount > times) {
        fail('Unexpected HTTP call #$callCount: ${request.method} ${request.url}');
      }
...
http.Request _verifyRequest(
  ...
  expect(request.method, equals(method), reason: 'HTTP method mismatch for $path');
  expect(
...
void main() {
  ...
  group('MarcarAsistencia', () {
    ...
    test('marcarSalida sends POST to /asistencia/marcar-salida with correct body',
        () async {
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request,
  ...
  group('Paradas CRUD', () {
    ...
    test('crearParada sends POST to /obras/10/paradas with correct body', () async {
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request,
              method: 'POST',
  ...
  group('Solicitudes', () {
    test('solicitarIngreso sends POST to /solicitudes with empleado_id and empresa_id',
        () async {
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request,
    ...
    test('obtenerSolicitudesEmpleado sends GET to /empleados/emp-1/historial_solicitudes',
        () async {
      final client = _mockResponse(200, [
    ...
    test('obtenerMisSolicitudes sends GET to /solicitudes/mis-solicitudes', () async {
      final client = _mockResponse(200, [
        {'id': 1, 'estado': 'pendiente'},
      ]);
      final api = ApiService.createForTesting(client: client);
    ...
    test('aceptarSolicitud sends PUT to /solicitudes/3/aceptar with obra_id', () async {
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request,
              method: 'PUT',
  ...
  group('Valoraciones', () {
    test('crearValoracion sends POST to /valoraciones with empresa_id, puntuacion, comentario',
        () async {
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request,
  ...
  group('Empleados', () {
    test('obtenerEmpleadosActuales sends GET to /empleados/actuales without empresa_id',
        () async {
      final client = _MockHttpClient(
        handler: (request) async {
          ...
          expect(request.url.queryParameters.containsKey('empresa_id'), isFalse);
          return http.Response(
            jsonEncode([{'id': 1, 'nombre': 'Juan'}]),
            200,
            headers: {'content-type': 'application/json'},
          );
        },
    ...
    test('actualizarEmpleado sends PUT to /empleados/1', () async {
      ...
      final result = await api.actualizarEmpleado(1, nombre: 'Juan Actualizado');
    ...
    test('desactivarEmpleado sends PUT to /empleados/1/desactivar', () async {
      ...
      final client = _MockHttpClient(
        handler: (request) async {
          _verifyRequest(request, method: 'PUT', path: '/empleados/1/desactivar');
          called = true;
          return http.Response('{}', 200, headers: {'content-type': 'application/json'});
        },
      );
    ...
    test('obtenerAsistenciasEmpleado sends GET to /asistencia/historial/1', () async {
      final client = _mockResponse(200, [
        {'id': 1, 'tipoMarcacion': 'entrada'},
      ]);
      final api = ApiService.createForTesting(client: client);
  ...
  group('Reportes', () {
    test('obtenerReporteAsistencia sends GET to /reportes/asistencia with query params',
        () async {
      final client = _MockHttpClient(
        handler: (request) async {
          expect(request.method, equals('GET'));
          ...
          expect(
              request.url.queryParameters['fecha_inicio'], equals('2026-01-01'));
          expect(
              request.url.queryParameters['fecha_fin'], equals('2026-01-31'));
